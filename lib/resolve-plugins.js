 ...
 * @property {Set<string>} explicitPlugins
...
async function internalResolvePluginOrder (plugins, loadPlugin, context) {
  ...
  for (let pluginName of plugins) {
    ...
    const peerDependency = pluginName.endsWith('~');
    ...
    if ((optionalDependency || peerDependency) && (pluginName.endsWith('?') || pluginName.endsWith('~'))) {
      throw new TypeError(`Invalid plugin name "${pluginName}": cannot combine dependency modifiers`);
    }
    ...
    if (!pluginDefinition) {
      ...
      } else if (peerDependency && !explicitPlugins.has(pluginName)) {
        orderedPlugins.add(false, { group: pluginName });
      } else {
    ...
    try {
      ...
      throw new Error(`Failed to add plugin "${pluginName}" to dependency graph`, { cause });
    }
...
export async function resolvePluginsInOrder (plugins, loadPlugin, allowOptionalDependencies) {
  ...
  /** @type {Set<string>} */
  const explicitPlugins = new Set(plugins.map(p => p.replace(/[?~]$/, '')));
  ...
  if (missingPlugins.size > 0) {
    ...
    throw new Error(`Required plugin${values.length > 1 ? 's' : ''} not found: "${values.join('", "')}". These are listed as dependencies but could not be loaded.`);
  }
